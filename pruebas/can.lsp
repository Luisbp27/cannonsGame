...
(defun rectangulo (x y)
    (cond
        ((= y 0) nil)
        (t (drawrel x 0) (moverel (- 0 x) 1) (rectangulo x (- y 1)))
    )
)

; Función para pintar el proyectil del cañón 1
(defun proyectilc1 (velocidad tiempo)
    (cond
...
(defun proyectilc2 (velocidad tiempo)
    (cond
        ;si choca contra el suelo de la tercera montaña = nil
...
(defun initcano ()
    (putprop 'cano (random (- (get 'escenari 'x2) 20)) 'posicio)
    (putprop 'cano (toradiant 45) 'angle)
    (putprop 'cano (+ 10 (get 'escenari 'y2)) 'altura)
)

(defun initcano2 ()
    (putprop 'cano2 (+ (random (-(- (get 'escenari 'x4)(get 'escenari 'x3)) 20)) (get 'escenari 'x3)) 'posicio) ; Normalización de la coordenada x3 para poder usar el random
...
; FUNCIONES DEL JUEGO
(defun simula (cano velocidad)
    (cond
        ((eq cano 'cano) (move (+(get 'cano 'posicio) 10) (+ (get 'escenari 'y2) 10))(proyectilc1 velocidad 0))
